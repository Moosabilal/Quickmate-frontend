...
const ServiceDetailsPage: React.FC = () => {
  ...
  const preSelectedProviderId = location.state?.preSelectedProviderId;
  ...
  const handleVerifyOtp = async (otpValue: string) => {
    ...
    try {
      setVerificationLoading(true);
      const res = await authService.verifyPhoneOtp(otpValue, phone);
      if (res.success || res.message) {
  ...
  const handlePayment = async (e: React.FormEvent, skipVerification: boolean = false) => {
    e.preventDefault()
    ...
    if (!isAuthenticated) {
      navigate(`/login?redirect=/service-detailsPage/${serviceId}`);
      return;
    }

    if (!isPhoneVerified && !skipVerification) {
    ...
    if (paymentMethod === PaymentMethod.WALLET) {
      ...
      const bookingResponse = await handleSubmit(e, skipVerification);
      try {
        if (!selectedProvider) {
    ...
    const options: RazorpayOptions = {
      ...
      handler: async function (paymentResponse: RazorpayResponse) {
        try {
          const bookingRes = await handleSubmit(e, skipVerification);
          if (!selectedProvider) {
  ...
  const handleSubmit = async (e: React.FormEvent, skipVerification: boolean = false) => {
    e.preventDefault();
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans text-gray-900 dark:text-white transition-colors duration-300">
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sticky top-24 border border-gray-100 dark:border-gray-700 transition-colors">
              <div className="mb-6 pb-4 border-b border-gray-100 dark:border-gray-700 text-center">
                ...
                <div className="mt-2 flex items-center justify-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full text-xs font-bold border border-blue-100 dark:border-blue-800">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  7-Day Service Validity Period
                </div>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">Complete the steps to schedule.</p>
              </div>

              <form className="space-y-5">
                <div className={`p-4 rounded-xl border transition-all duration-300 ${isStep1Complete
                ...
                <div className={`p-4 rounded-xl border transition-all duration-300 ${isStep2Complete
                  ? 'border-green-500 bg-green-50/30 dark:bg-green-900/10'
                ...
                {isStep2Complete && (
                  <div className={`p-4 rounded-xl border transition-all duration-300 ${isStep3Complete
                    ? 'border-green-500 bg-green-50/30 dark:bg-green-900/10'
                    : 'border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-white dark:hover:bg-gray-800'
                    }`}>
                    <h3 className="text-base font-semibold text-gray-800 dark:text-gray-200 mb-3 flex items-center justify-between">
                  ...
                  <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm transition-all duration-300 hover:border-blue-400 dark:hover:border-blue-500">
                    ...
                    <div className="space-y-4 mb-6">
                      <textarea
                        ...
                        className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none transition-all text-sm bg-gray-50/50 dark:bg-gray-700/30 text-gray-900 dark:text-white placeholder-gray-400 focus:bg-white dark:focus:bg-gray-800"
                      />
                    </div>
                    ...
                    <button type="submit" onClick={handlePayment} disabled={!fullName && isAuthenticated} className="w-full bg-gradient-to-r from-green-500 to-green-600 text-white font-bold py-3.5 px-4 rounded-xl text-base hover:from-green-600 hover:to-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all shadow-lg transform active:scale-[0.98]">
                      {!isAuthenticated
                        ? <span className="flex items-center justify-center gap-2"><LogIn className="w-4 h-4" />Login to Confirm Booking • ₹{selectedProvider.price}</span>
                        : isPhoneVerified
      ...
      {providerPopup && <ProviderPopup setSelectedProvider={setSelectedProvider} providerPopup={providerPopup} selectedProvider={selectedProvider} setProviderPopup={setProviderPopup} serviceId={serviceId || ''} selectedDate={selectedDate} selectedTime={selectedTime} latitude={selectedAddress?.locationCoords ? Number(selectedAddress.locationCoords.split(',')[0]) : 0} longitude={selectedAddress?.locationCoords ? Number(selectedAddress.locationCoords.split(',')[1]) : 0} radiusKm={radius} preSelectedProviderId={preSelectedProviderId} />}
      <AddressPopup addressPopup={addressPopup} setAddressPopup={setAddressPopup} selectedAddress={selectedAddress} handleAddressConfirm={handleAddressConfirm} setShowAddAddress={setShowAddAddress} showAddAddress={showAddAddress} newAddress={newAddress} setNewAddress={setNewAddress} handleAddAddress={handleAddAddress} serviceId={serviceId || ''} isGuest={!isAuthenticated} />
      ...
      <PhoneVerificationModal
        ...
        description="Please enter your 10-digit mobile number to verify your identity and confirm your booking."
